...
def train(args, device, vocab, train_file, total_train_n, total_lines,
    ...
    if args.mode == 'supervised':
        ...
        ref_vocab, loaded_vecs = gh.load_word_vectors(args.ref_embs)
        ...
        # note - these vecs have been normed, mean-centered, normed.
        ref_vocab, loaded_vecs = gh.load_word_vectors(
        ...
        for idx, (context_idxs, target_idxs, labels) in enumerate(train_dataloader):
            ...
            if use_iso_loss:
                ...
                if (args.loss == 'wass' or args.loss == 'procwass'):
                    ...
                    iso_loss = gh.compute_distance(loaded_dist, model_dist, device=device)
                elif args.loss == 'rs':
                    ...
                    # vecs we can use?
                    iso_loss = gh.diffble_evs_distance(model_vecs_tmp,
                            loaded_vecs, device)
